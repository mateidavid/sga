...
int main(int argc, char* argv[])
{
    ...
    // print options
    logger("main", info) << variables_map_converter::to_ptree(global::vm, ac);
    ...
    if (not rf_reads_fn.empty())
    {
        ...
        if (not bf_save_fn.empty())
        {
            ...
            logger("main", info) << "saving Bloom Filter to file: " << bf_save_fn << endl;
            global::bf_p->save(bf_save_os);
            logger("main", info) << "done saving Bloom Filter" << endl;
        }
    }
    else
    {
        ...
        logger("main", info) << "loading Bloom Filter from file: " << bf_load_fn << endl;
        global::bf_p->load(bf_load_is);
        logger("main", info) << "done loading Bloom Filter" << endl;
    }
