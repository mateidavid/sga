...
private:
    ...
    void merge_reads(Contig_Entry_BPtr ce_bptr, const RE_OSet& common_support) const;
    ...
    Read_Chunk_BPtr merge_contig_chunks(const RC_OSet& crt_chunks, Read_Entry_BPtr m_re_bptr) const;
    Read_Chunk_BPtr merge_unmappable_chunks(const RC_OSet& crt_chunks, Read_Entry_BPtr m_re_bptr) const;
    pair< RC_OSet, RC_OSet > advance_chunks(const RC_OSet& crt_rc_oset, bool direction) const;
}; // class Read_Merger
