...
tuple< Size_Type, bool> Graph::visit_contig(const Contig_Entry* ce_cptr, bool dir)
{
    ...
    return make_tuple(ce_cptr->len() + tmp, false);
}
                        ...
                        list< tuple< const Contig_Entry*, size_t, size_t, bool > >& l, bool& cycle)
{
    while (true)
    {
        ...
        {
            auto t = make_tuple(ce_cptr, ce_cptr->len(), ce_cptr->len(), dir == ce_endpoint);
...
void Graph::print_supercontig_lengths(ostream& os)
{
    for (auto ce_it = _ce_cont.begin(); ce_it != _ce_cont.end(); ++ce_it)
    {
        ...
        os << supercontig_left_len + supercontig_right_len + ce_cptr->len() << '\n';
    }
}
*/
...
void Graph::print_supercontig_lengths(ostream& os)
{
    for (auto ce_it = _ce_cont.begin(); ce_it != _ce_cont.end(); ++ce_it)
    {
        ...
        Size_Type scon_len = ce_cptr->len();
        ...
        for (int side = 0; side < 2; ++side)
        {
            if (side == 1)
            {
                ...
                os << '(' << ce_cptr->get_contig_id() << ',' << ce_cptr->len() << ",0)";
            }
            for (auto it = l[side].begin(); it != l[side].end(); ++it)
            {
                ...
                if (tmp_ce_cptr != NULL)
                {
                    os << '(' << tmp_ce_cptr->get_contig_id() << ',' << tmp_ce_cptr->len() << ',' << int(flip) << ')';
                }
                else
                {
    ...
    map< string, tuple< size_t, string > >& seq_bseq_map)
{
    ...
    */
    ...
    vector< map< string, size_t > > kmer_map_v(seq_v.size());
    for (size_t k = 0; k < seq_v.size(); ++k)
    {
        const string& seq = seq_v[k];
        map< string, size_t >& m = kmer_map_v[k];
        for (size_t i = 0; i + 3 < seq.size(); ++i)
        {
            if (m.count(seq.substr(i, 3)) == 0)
            {
                m[seq.substr(i, 3)] = 1;
            }
            else
    ...
    auto get_dist = [] (const map< string, size_t >& km1, const map< string, size_t >& km2)
    {
    ...
    Contig_Entry_CBPtr ce_next_cbptr, bool same_orientation)
{
    ...
    for (const auto& rc_cbptr : out_chunks_v)
    {
        ...
        string seq = rc_next_cbptr->get_seq().revcomp(not r_right);
    ...
    for (size_t i = 0; i < unmappable_chunks_v.size(); ++i)
    {
        ...
        string seq = old_rc_bptr->get_seq().revcomp(not r_right);
        size_t bseq_idx;
        string cigar_string;
    ...
    while (getline(is, line))
    {
        ...
        else if (cmd == "h" || cmd == "help")
        {
            os << "available commands:\n"
               ...
               << "  quit|q : quit\n"
               << "  ptree|pt [read|contig|chunk] id : print as ptree\n"
               << "  prettyprint|pp [read|contig|chunk|res|rcs] id : pretty print\n"
               << "  outchunks|oc ce c_right [unmappable_policy [ignore_thres]]\n";
        }
