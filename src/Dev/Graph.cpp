                        ...
                        const string& cigar_string)
{
    ...
    auto region_cont = find_overlapping_regions(re1_bptr, cigar.rf_start(), cigar.rf_end());
    if (not region_cont.empty())
    {
        _u.unmap(re1_bptr, move(region_cont));
    }
    // check read entries
...
Range_Cont
Graph::find_overlapping_regions(Read_Entry_CBPtr re_cbptr, Size_Type r_start, Size_Type r_end) const
{
    ...
    while (pos < r_end)
    {
        ...
        find_overlapping_regions(rc_cbptr, region_cont);
    }
    return region_cont;
}

void
Graph::find_overlapping_regions(Read_Chunk_CBPtr orig_rc_cbptr, Range_Cont& region_cont) const
{
...
void Graph::load_aux_bwt(const string& aux_bwt_file, unsigned read_len, double coverage)
{
...
Graph::find_reads_with_seq(const Seq_Proxy_Type& seq, unsigned max_count) const
{
    ...
    for (auto ce_bptr : ce_cont() | referenced)
    {
        ...
        for (auto mut_bptr : ce_bptr->mut_cont() | referenced)
        {
            ...
            for (int al = 0; al < 2; ++al)
            {
                ...
                auto haploid_coverage = get_aux_coverage(s.size());
                if (haploid_coverage > 10.0)
                {
                    int cn = round((double)cnt / haploid_coverage);
                    if (0 <= cn and cn <= 2)
                    {
                ...
                else // haploid coverage too low
                {
                    mut_bptr->copy_num(al) = -3;
                }
            } // for al
        } // for mut_bptr
    } // for ce_bptr
...
void Graph::compute_aux_coverage()
{
    ...
    {
        LOG("Graph", warning)
            << "optimal seq_len[=" << seq_len << "] is large compared to read_len=[" << _aux_read_len << "]; "
    ...
    LOG("Graph", info) << ptree("end")
        ...
        .put("base_coverage", _aux_coverage);
