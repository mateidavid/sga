...
{
    ASSERT(orig_rg.begin() <= orig_rg.end());
    Range_Type rg(max(orig_rg.begin(), on_contig? get_c_start() : get_r_start()),
    ...
    ASSERT(ret_rg_start <= ret_rg_end or (rg.begin() == rg.end() and not rg_start_maximal and not rg_end_maximal));
