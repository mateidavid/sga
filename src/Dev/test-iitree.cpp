...
bool operator < (const B& lhs, const B& rhs) { return lhs._start < rhs._start; }
...
struct Node_Traits
{
    ...
    static void recompute_data(node_ptr node)
    {
        size_t tmp = node->_end;
        if (get_left(node))
        {
            tmp = max(tmp, get_left(node)->_max_end);
        }
        if (get_right(node))
        {
            tmp = max(tmp, get_right(node)->_max_end);
        }
        node->_max_end = tmp;
    }
...
typedef boost::intrusive::multiset< B, boost::intrusive::value_traits< Value_Traits < B > > > iitree_type;
typedef boost::intrusive::bstree_algorithms< Node_Traits< B > > bstree_algorithms;
